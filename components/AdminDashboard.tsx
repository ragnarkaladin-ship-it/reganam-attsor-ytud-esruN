...
import { optimizeRoster, summarizeMessages, generateSmsNotification } from '../services/geminiService';
                    ...
                    Object.entries(
                      ...
                      <div key={ward} className="space-y-6">
                        ...
                        {(wardMessages as Message[]).map((msg: Message) => {
                          ...
                          return (
                            <div key={msg.id} className={`p-8 rounded-[3rem] border transition-all ${isLeaveRequest ? (status === 'approved' ? 'bg-emerald-50 border-emerald-100' : status === 'rejected' ? 'bg-red-50 border-red-100' : 'bg-indigo-50 border-indigo-100 shadow-lg shadow-indigo-100/50') : 'bg-slate-50 border-slate-100'}`}>
                              <div className="flex justify-between items-start mb-4">
                                <div className="flex items-center gap-4">
                                   <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-lg ${isLeaveRequest ? 'bg-white text-indigo-600 shadow-sm' : 'bg-indigo-900 text-white'}`}>{sender?.name[0] || 'A'}</div>
                                   <div>
                              ...
                              {isLeaveRequest && status === 'pending' && onProcessLeave && (
                                <div className="flex gap-4">
                                  <button onClick={() => onProcessLeave(msg.id, 'approved')} className="bg-indigo-600 text-white px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100"><Check size={16} /> Approve Leave</button>
